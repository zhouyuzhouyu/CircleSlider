...
#define kWidth_BG 40
//内环 的宽度
#define kWidth_LINE 30
//外圈预留宽度
//#define kWidth_Wai 10
#define kSliderSize 200
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        self.radius = frame.size.width/2.0f - kWidth_BG/2.0f;
        self.angle = 360;
        ...
        self.textField = [[UITextField alloc]initWithFrame:CGRectMake((frame.size.width  - fontSize.width) /2,
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetLineWidth(ctx, kWidth_BG);
    CGContextDrawPath(ctx, kCGPathStroke);
    
    /** Create THE MASK Image **/
    UIGraphicsBeginImageContext(CGSizeMake(kSliderSize,kSliderSize));
    CGContextRef imageCtx = UIGraphicsGetCurrentContext();
    
    CGContextAddArc(imageCtx, self.frame.size.width/2  , self.frame.size.height/2, self.radius, 0, ToRad(self.angle), 0);
    ...
    //define the path
    CGContextSetLineWidth(imageCtx, kWidth_LINE);
    ...
    CGContextAddArc(ctx, self.frame.size.width/2  , self.frame.size.height/2, self.radius+kWidth_BG/2, 0, ToRad(-self.angle), 1);
    ...
    CGContextFillEllipseInRect(ctx, CGRectMake(handleCenter.x, handleCenter.y, kWidth_LINE, kWidth_LINE));
...
-(CGPoint)pointFromAngle:(int)angleInt{
    ...
    //Circle center
    CGPoint centerPoint = CGPointMake(self.frame.size.width/2 - kWidth_LINE/2, self.frame.size.height/2 - kWidth_LINE/2);
